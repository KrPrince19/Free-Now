...
import {
  Sparkles, MessageCircle, Coffee, Zap, AlertCircle, BellRing,
      ...
      <AnimatePresence>
        {statusToast && (
          <motion.div initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }}
            className={`fixed top-24 right-6 z-[100] p-5 rounded-3xl shadow-2xl border flex items-center gap-4 min-w-[320px] backdrop-blur-md ${statusToast.isHeartbreak ? "bg-slate-950/95 border-rose-500/50 text-white" :
              isDarkMode ? "bg-[#1a1a21]/95 border-white/10 text-white" : "bg-rose-500/95 border-rose-400 text-white"
              }`}
          >
            ...
            <div>
              <p className={`text-[10px] font-black uppercase tracking-widest mb-0.5 ${statusToast.isHeartbreak ? "text-rose-400" : "opacity-60"}`}>
                {statusToast.isHeartbreak ? "Heartbreak" : "Vibe Update"}
              </p>
            ...
            <div className="flex items-center justify-between relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className={`font-bold text-lg ${isDarkMode ? 'text-white' : 'text-white'}`}>{incomingRequest.senderName} wants to chat!</p>
                </div>
              </div>
              <div className="bg-white/5 px-3 py-1 rounded-full text-xs font-mono font-bold text-rose-400 border border-white/5">
              ...
              {filteredUsers.length > 0 ? (
                filteredUsers.map((u) => (
                  <motion.div key={u.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
                    className={`p-6 rounded-[2.5rem] flex items-center justify-between shadow-xl transition-all border group hover:scale-[1.02] ${isDarkMode ? 'bg-[#111116] border-white/5 shadow-black/20' : 'bg-white border-slate-50'}`}
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-gradient-to-tr from-indigo-500 to-rose-500 rounded-2xl flex items-center justify-center text-white text-lg font-black shadow-lg shadow-indigo-500/20">
                        {u.name ? u.name[0] : "?"}
                      </div>
                      <div>
                        <h4 className={`font-black text-base ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{u.name}</h4>
                        <div className="flex items-center gap-1 text-xs font-bold bg-gradient-to-r from-indigo-500 to-rose-500 bg-clip-text text-transparent italic mt-1">
                          <Coffee size={14} className="text-rose-400" /> "{u.status}"
                        </div>
                      </div>
                    </div>
                    <button onClick={() => handleSayHi(u.id)} className={`px-6 py-3 rounded-2xl transition-all shadow-lg active:scale-95 flex items-center gap-2 ${isDarkMode ? 'bg-white text-black font-black' : 'bg-slate-900 text-white font-black hover:bg-rose-500'}`}>
...
function EmptyContent({ isSelfActive, isDark }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-[50vh] flex flex-col items-center justify-center text-center">
      <div className={`w-24 h-24 rounded-[2rem] flex items-center justify-center shadow-2xl mb-8 border transition-all ${isDark ? 'bg-white/5 border-white/5 text-indigo-400' : 'bg-white border-slate-100 text-rose-400'}`}>
        <Sparkles size={40} />
      </div>
